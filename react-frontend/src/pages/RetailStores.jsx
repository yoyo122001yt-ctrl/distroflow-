...
const emptyStore = { business_name: '', code: '', phone: '', email: '', address: '', city: '', state: '', store_type: 'retail', credit_limit: 0, status: 'active' }
...
export default function RetailStores() {
  ...
  const fetchStores = async () => {
    ...
    } catch { toast.error(t('stores.loadFailed')) }
    finally { setLoading(false) }
  }

  const filtered = typeFilter ? stores.filter(s => s.store_type === typeFilter) : stores
  ...
  const columns = [
    { header: t('stores.name'), accessor: 'business_name', render: (r) => (
    ...
    { header: t('stores.type'), accessor: 'store_type', render: (r) => (
      <span className="badge bg-gray-100 text-gray-700 capitalize">{r.store_type === 'retail' ? t('stores.retail') : r.store_type === 'wholesale' ? t('stores.wholesale') : r.store_type === 'distributor' ? t('stores.distributor') : r.store_type}</span>
    )},
    ...
    { header: t('stores.creditBalance'), accessor: 'credit_limit', render: (r) => (
      <span className="font-medium">{formatCurrency(r.current_balance || 0)} / {formatCurrency(r.credit_limit)}</span>
    )},
    ...
    <div>
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="text-sm text-gray-500">{t('stores.filterType')}</span>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? t('stores.editStore') : t('stores.addStore')} size="lg">
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.nameLabel')}</label>
              <input className="input-field" value={form.business_name} onChange={e => setForm(f => ({ ...f, business_name: e.target.value }))} required /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.codeLabel')}</label>
              <input className="input-field" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} required /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.typeLabel')}</label>
              <select className="select-field" value={form.store_type} onChange={e => setForm(f => ({ ...f, store_type: e.target.value }))}>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.statusLabel')}</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.phoneLabel')}</label>
              <input className="input-field" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.emailLabel')}</label>
              <input type="email" className="input-field" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} /></div>
            <div className="col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.addressLabel')}</label>
              <input className="input-field" value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.cityLabel')}</label>
              <input className="input-field" value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.stateLabel')}</label>
              <input className="input-field" value={form.state} onChange={e => setForm(f => ({ ...f, state: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('stores.creditLimitLabel')}</label>

...
export default function Drivers() {
  ...
  const fetchDrivers = async () => {
    ...
    } catch { toast.error(t('drivers.loadFailed')) }
  ...
  const viewSettlements = async (driver) => {
    ...
    } catch { toast.error(t('drivers.settlementsLoadFailed')) }
  }
  ...
  const columns = [
    ...
    { header: t('drivers.payRate'), accessor: 'pay_rate', render: (r) => `${formatCurrency(r.pay_rate)}/${r.pay_type === 'per_delivery' ? t('drivers.perDelivery') : r.pay_type === 'hourly' ? t('drivers.hourly') : t('drivers.salary')}` },
  ...
  return (
    <div>
      ...
      <div className="card">
        <DataTable columns={columns} data={drivers} loading={loading} searchPlaceholder={t('drivers.search')} />
      </div>

      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? t('drivers.editDriver') : t('drivers.createDriver')} size="lg">
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.nameLabel')}</label>
              <input className="input-field" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.phoneLabel')}</label>
              <input className="input-field" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.emailLabel')}</label>
              <input type="email" className="input-field" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.licenseLabel')}</label>
              <input className="input-field" value={form.license_number} onChange={e => setForm(f => ({ ...f, license_number: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.vehiclePlateLabel')}</label>
              <input className="input-field" value={form.vehicle_plate} onChange={e => setForm(f => ({ ...f, vehicle_plate: e.target.value }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.payTypeLabel')}</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.payRateLabel')}</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('drivers.statusLabel')}</label>
            ...
            {settlements.map((s, i) => (
              <div key={s.id || i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{t('drivers.deliveriesKm', { count: s.deliveries_completed || 0, km: s.total_distance || 0 })}</p>
                </div>

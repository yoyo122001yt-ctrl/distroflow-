  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500 bg-white px-3 py-1.5 rounded-lg border">
            {t('settlements.pending', { amount: formatCurrency(totalPending) })}
          </span>
      ...
      <div className="card">
        ...
        <DataTable columns={columns} data={filtered} loading={loading} searchPlaceholder={t('settlements.search')} />
      </div>

      <Modal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} title={t('settlements.details')} size="lg">
        {selectedSettlement && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-500">{t('settlements.driverColon')}</span> <span className="font-medium">{selectedSettlement.driver_name}</span></div>
              <div><span className="text-gray-500">{t('settlements.periodColon')}</span> {selectedSettlement.period}</div>
              <div><span className="text-gray-500">{t('settlements.statusColon')}</span> <span className={`badge ${getStatusBadgeClass(selectedSettlement.status)}`}>{selectedSettlement.status}</span></div>
              <div><span className="text-gray-500">{t('settlements.totalPayColon')}</span> <span className="font-bold">{formatCurrency(selectedSettlement.total_pay || selectedSettlement.amount || 0)}</span></div>
            </div>

...
// Warehouse Tracking Page (supports session auth or ?token= query param, requires admin/warehouse_manager/manager)
Route::middleware(['auth.token', 'auth', \App\Http\Middleware\CheckRole::class . ':admin,warehouse_manager,manager'])->group(function () {
    Route::get('/warehouse/tracking', [App\Http\Controllers\Warehouse\TrackingController::class, 'showTrackingDashboard'])->name('warehouse.tracking');
    Route::get('/warehouse/active-drivers', [App\Http\Controllers\Warehouse\TrackingController::class, 'getActiveDrivers'])->name('warehouse.tracking.drivers');
    Route::get('/warehouse/driver/{driverId}/route', [App\Http\Controllers\Warehouse\TrackingController::class, 'getDriverRoute'])->name('warehouse.tracking.route');
});

...
<div class="container">
    ...
    <div class="card">
        ...
        <dl class="detail-grid">
            ...
            <div><dt>Order Date</dt><dd>{{ $order->order_date->format('d M Y, h:i A') }}</dd></div>
            <div><dt>Total Amount</dt><dd>{{ number_format($order->total_amount, 2) }} EGP</dd></div>
            <div><dt>Requested Delivery</dt><dd>{{ $order->requested_delivery_date ? $order->requested_delivery_date->format('d M Y') : 'Not specified' }}</dd></div>
            @if($order->notes)
            <div style="grid-column: 1 / -1;"><dt>Notes</dt><dd>{{ $order->notes }}</dd></div>
            @endif
        </dl>
    </div>
    ...
    <div class="card">
        <h3 style="margin-bottom: 1rem;">Deliveries</h3>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0; border-bottom: 1px solid #f3f4f6;">
            ...
            <div style="text-align: right;">
                ...
                <div style="margin-top: 0.25rem;">
                    <a href="{{ route('store.track', $delivery->id) }}" style="font-size: 0.875rem;">Track</a>
                </div>
            </div>
        </div>
        @endforeach
    ...
    <div style="margin: 1rem 0;">
        <a href="{{ route('store.orders') }}" style="color: #2563eb;">&larr; Back to Orders</a>
    </div>
</div>
@endsection

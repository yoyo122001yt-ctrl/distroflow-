...
@section('title', __('store.trackDelivery'))
...
<div class="store-header">
    <div class="container">
        <h1>{{ __('store.trackDelivery') }}</h1>
        <p>{{ __('store.deliveryNo', ['id' => $delivery->id]) }}</p>
    </div>
</div>
...
<div class="container">
    ...
    <div class="driver-info">
        ...
        <div>
            ...
            <div class="phone">{{ $delivery->driver->phone ?? __('store.noPhone') }}</div>
        </div>
    </div>
    @endif
    ...
    <div id="map">
        <div style="display: flex; height: 100%; align-items: center; justify-content: center; background: #f3f4f6; color: #6b7280;">
            {{ __('store.mapPlaceholder') }}
        </div>
    </div>
    <div style="font-size: 0.875rem; color: #6b7280; margin-top: 0.5rem;">
        @if($driverLocation)
        {{ __('store.lastUpdated', ['time' => $driverLocation->created_at->diffForHumans()]) }}
        @else
        {{ __('store.noLocationData') }}
        @endif
    </div>

    <div class="card">
        <h3>{{ __('store.deliveryStatus') }}</h3>
        <ul class="status-timeline">
            <li class="{{ in_array($delivery->status, ['assigned','in_transit','completed']) ? 'completed' : ($delivery->status === 'pending' ? 'active' : '') }}">
                <div>
                    <strong>{{ __('store.orderPlaced') }}</strong>
            ...
            <li class="{{ in_array($delivery->status, ['in_transit','completed']) ? 'completed' : ($delivery->status === 'assigned' ? 'active' : '') }}">
                <div>
                    <strong>{{ __('store.driverAssigned') }}</strong>
            ...
            <li class="{{ $delivery->status === 'completed' ? 'completed' : ($delivery->status === 'in_transit' ? 'active' : '') }}">
                <div>
                    <strong>{{ __('store.inTransit') }}</strong>
            ...
            <li class="{{ $delivery->status === 'completed' ? 'completed' : '' }}">
                <div>
                    <strong>{{ __('store.delivered') }}</strong>
    ...
    <div style="margin: 1rem 0;">
        <a href="{{ route('store.order.detail', $delivery->sales_order_id) }}" style="color: #2563eb;">{{ __('store.backToOrder') }}</a>
    </div>
</div>
@endsection

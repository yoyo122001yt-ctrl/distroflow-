...
@section('title', __('store.dashboard'))
...
<div class="store-header">
    <div class="container">
        <h1>{{ __('store.welcome', ['name' => Auth::user()->name]) }}</h1>
        <p>{{ $store?->business_name ?? __('store.dashboard') }}</p>
    </div>
</div>

<div class="container">
    <nav class="store-nav">
        <a href="{{ route('store.dashboard') }}" class="active">{{ __('store.dashboard') }}</a>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
        <div class="card">
            <h2 style="margin-bottom: 1rem;">{{ __('store.recentOrders') }}</h2>
            ...
            <a href="{{ route('store.orders') }}" style="display: inline-block; margin-top: 0.5rem;">{{ __('store.viewAllOrders') }}</a>
            @else
            <p style="color: #6b7280;">{{ __('store.noOrdersYet') }}</p>
            @endif
        </div>

        <div class="card">
            <h2 style="margin-bottom: 1rem;">{{ __('store.popularProducts') }}</h2>
            ...
            <div class="product-grid">
                ...
                <div class="product-card">
                    ...
                    <form action="{{ route('store.cart.add') }}" method="POST" style="margin-top: 0.5rem;">
                        ...
                        <button type="submit" style="background: #2563eb; color: #fff; border: none; padding: 0.375rem 1rem; border-radius: 0.375rem; cursor: pointer; font-size: 0.875rem;">{{ __('store.addToCart') }}</button>
                    </form>
                </div>
                @endforeach
            </div>
            @else
            <p style="color: #6b7280;">{{ __('store.noProductsAvailable') }}</p>
            @endif
        </div>
    </div>
</div>
